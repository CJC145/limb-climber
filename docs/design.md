...
**The chord works in either order.** *Changed after the phase 5 playtest.* This used to be decided at the moment the limb key went down, so `Shift` had to come first and the reverse order silently became a reach, which dropped the hold. No player would discover that. Now a pull engages whenever both keys are held on a gripped limb, whichever came first.

Shift first resolves at the press, as before. Limb key first is the hard case, because a held limb key is what releases a grip, and the server drops the grip on the first held intent it receives. So on a gripped limb, a bare limb key waits `Config.Input.PullGraceSeconds` on the client and sends nothing. `Shift` arriving inside that window makes it a pull; otherwise it becomes the reach it always was. The cost is that delay on every reach that starts from a hold, which is why it is short. A free limb has no grip to protect and reaches at once. A tap on a gripped limb shorter than the window now does nothing, where before it dropped the grip for a frame and re-gripped wherever the hand was.

**Letting go of either key ends the pull.** Letting go of `Shift` while the limb key is still down leaves the limb gripped and idle until the limb key comes up. It never falls back into a reach, because a reach begins by dropping the hold that was being hauled on, and pressing `Shift` again does not restart it; the limb key has to be pressed again. `Space` carries the modifier like any other limb key, so a solo player's alias works in both modes rather than only one. The HUD shows the chord, because a modifier nobody is told about is a feature nobody has.
